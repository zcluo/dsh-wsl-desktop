...
import { checkLinuxPath, defaultDistro, listDistros, listLinuxDir, runWslShell } from './wsl/world.js'
...
async function materializePresets(ctx, distro) {
  ...
  for (const preset of roster) {
    ...
    // deriving from it; the orphan sweep below withdraws any earlier variant.
  ...
  for (const entry of await readdir(root, { withFileTypes: true }).catch(() => [])) {
    ...
    const decision = sweepDecision({
      ...
      expected: expected.has(entry.name),
...
const METHODS = {
  ...
  checkPath: async ({ distro, path }) => {
    ...
    return { ...facts, uncPath: joinWslUnc(distro, path) }
  },
...
async function devToken() {
  ...
  try {
    ...
    // Absent (or unreadable) means this is the first use: mint one below.
  }
...
export function apply(ctx, config) {
  ...
  // A WSL workspace's session opens on the host preset, so the execution world
  // is bound from the host rather than the browser.
  //
...
async function bindWslSession(ctx, summary) {
  ...
  if (agent === undefined) {
    bindingLog.push({ sessionId: String(summary?.sessionId), cwd, ok: false, error: 'agent 尚未注册' })
    return
  }
...
async function bindWslPreset(ctx, agent, cwd, sessionId) {
  ...
  try {
    ...
    bindingLog.push({ sessionId, cwd, from, to: chosen, ok: true })
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error)
    bindingLog.push({ sessionId, cwd, from, to: wanted, ok: false, error: reason })
