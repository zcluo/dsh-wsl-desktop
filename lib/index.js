  ...
  selftest: async ({ preset = 'wsl-standard', cwd }) => {
    ...
    try {
      ...
      if (shell !== undefined) {
        ...
        // 0.1.7 renamed run → execute; dual-dispatch for older hosts.
        const result = await (typeof shell.execute === 'function' ? shell.execute(spec) : shell.run(spec))
        record('shell.run', {
...
export function apply(ctx, config) {
  ...
  ctx.inject(['agentPresets'], (scope) => {
    // On plugin teardown, withdraw every variant this plugin registered —
    // otherwise disabling the plugin leaves wsl-* presets in the host
    // registry whose rows name modules of a disabled plugin.
    scope.effect(() => () => {
