 ...
 *
 * The host half binds the WSL agent preset when a session is created, so this
 * side never touches preset APIs.
 */
window.__ModuleLoader__.load({
  ...
  factory() {
     ...
     *
     * Every branch here is idempotent and no branch removes the node: the
     * observer watches `childList`, so a `remove()` here would be its own next
     * trigger and the two would loop. Not fitting is expressed as `display`,
     * which is not a childList mutation.
     */
    function sync() {
