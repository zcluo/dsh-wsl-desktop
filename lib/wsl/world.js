...
import { isWindowsPathShaped, mntToWindowsPath, parseWslUnc, shellQuote, windowsToMntPath } from './paths.js'
...
export async function listLinuxDir(distro, linuxPath) {
  ...
  const result = await runWslShell({ distro, linuxCwd: '/', command: script, timeoutMs: 30_000 })
