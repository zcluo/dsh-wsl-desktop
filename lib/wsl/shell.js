...
/**
 * Build the exact `wsl.exe` argv for one command.
 *
 * A login shell may run profile scripts that reset the working directory, so
 * the requested directory is re-asserted inside the command itself.
 * @param {{ distro: string, linuxCwd: string }} plan - the execution plan.
 * @param {string} command - shell source from the caller.
 * @param {{ username?: string, wslPath?: string, loginShell?: boolean }} options - executor settings.
 * @returns {string[]} the argv handed to `ctx.subprocess`.
 */
/**
...
export class WslShellExecutor extends ShellExecutor {
  ...
  async confinementFor(spec) {
    ...
    if (runner !== RUNNER_SUDO_UNSHARE) {
      throw new SandboxUnavailableError(
        mode,
        'wsl-sandbox: 发行版内没有可用的约束运行器（需要免密 sudo、unshare 与 setpriv）',
      )
    }
    ...
    return {
      ...
      // code rather than running anyway), but it cannot govern two things the
      ...
      // and interop — a confined command may still ask `wsl.exe` to run a
      // Windows program that writes files. Reporting `full` here was wrong.
      sandbox: { mode, denied: false, enforcement: 'partial' },
    }
  }
  ...
  async run(spec) {
    ...
    try {
      ...
      const collectedErr = WslShellExecutor.settled(stderr)
      return {
        ...outcome,
        timedOut: deadline.timedOut(),
        aborted: spec.signal?.aborted === true && !deadline.timedOut(),
        timeoutMs: spec.timeoutMs,
        stdout: WslShellExecutor.settled(stdout),
        stderr: collectedErr,
        sandbox: { ...sandbox, denied: WslShellExecutor.wasDenied(outcome, collectedErr.text) },
      }
    } finally {
      deadline.dispose()
    }
