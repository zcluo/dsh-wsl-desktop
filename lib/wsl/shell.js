...
export class WslShellExecutor extends ShellExecutor {
  ...
  async confinementFor(spec) {
    ...
    // NO_NEW_PRIVS on the drop closes the retained-sudo self-escape: without
    // it the session user's passwordless sudo grant (the same primitive the
    // runner uses) can re-invoke an unfenced unshare or lift the read-only
    // remount from inside the fence.
    const noNewPrivs = await detectNoNewPrivs({
      ...probeOptions,
      ...this.config.username !== undefined ? { username: this.config.username } : {},
    })
   ...
   *
   * 0.1.7 renamed the abstract method `run` → `execute` (tool-bash now calls
   * `ctx.shell.execute(...)`); `run` is kept as an alias so the provider also
   * satisfies pre-0.1.7 hosts and older callers.
   * @param {object} spec - a resolved spec.
