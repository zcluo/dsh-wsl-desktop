...
export function buildVariantPlugins(basePlugins, { subprocessPath, shellPath, fsPath, distro, sourceDir }) {
  ...
  for (const row of basePlugins) {
    if (row !== null && typeof row === 'object' && row.id !== undefined && WORLD_ROWS.has(row.id)) {
      removed.push(row.id)
      if (row.id === 'str-replace-editor') includeEditor = true
      continue
    }
    if (row !== null && typeof row === 'object') {
      if (row.id === 'persona') appendPersonaSuffixObject(row)
      if (sourceDir !== undefined && typeof row.name === 'string' && row.name.startsWith('./')) {
        row.name = join(sourceDir, row.name.slice(2)).replace(/\\/g, '/')
      }
    }
    kept.push(row)
  }
