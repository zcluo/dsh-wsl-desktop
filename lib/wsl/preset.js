...
export function buildVariantPlugins(basePlugins, { subprocessPath, shellPath, fsPath, distro, sourceDir }) {
  ...
  for (const row of basePlugins) {
    ...
    // would compound on every re-registration). Clone shallowly per row, and
    // clone `config` too when it is a plain object (structuredClone is not
    // safe: loader rows may carry JsExpr nodes).
    const clone = { ...row }
    if (clone.config !== null && typeof clone.config === 'object') clone.config = { ...clone.config }
