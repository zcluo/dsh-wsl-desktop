...
export async function resolveIdentity({ distro, username, run }) {
  ...
  const result = await run({
    ...
    command: 'id -u; id -g; printf "%s\\n" "$HOME"; id -un',
    timeoutMs: 15_000,
  })
  const [uid, gid, home, name] = result.stdout.trim().split('\n')
...
export async function detectRunner({ distro, username, run }) {
  ...
  try {
    ...
    runner = result.stdout.includes('yes') ? RUNNER_SUDO_UNSHARE : null
  } catch {
...
function readOnlySweep(keep) {
  const exempt = [...new Set([...keep, ...KERNEL_SURFACES])].map((path) => shellQuote(path)).join('|')
  return [
    // Line-driven, not word-split: `$(…)` in a for-loop would split a mount
    // target containing whitespace into two bogus targets, and both the sweep
    // and the post-condition would silently skip the real one. The `grep || …`
    // guard keeps a hypothetical all-exempt listing (grep exits 1) from
    // failing the pipeline under `set -o pipefail`.
    `findmnt -rno TARGET | { grep -Ev '^(${exempt})$' || true; } | while IFS= read -r target; do`,
...
function postConditions(keep) {
  ...
  return [
    ...
    // Same line-driven loop as the sweep. The trailing `true` matters: the
    // while is the LAST element of a pipeline, so a body that ends with a
    // failing `[ -w ]` (the normal case — every swept target IS read-only)
    // would otherwise fail the whole pipeline under `set -o pipefail` and
    // abort the script before the command runs. A real `fail` still exits 97
    // from inside the loop, which pipefail turns into the script's own setup
    // failure — the marker and exit code still surface.
    `findmnt -rno TARGET | { grep -Ev '^(${exempt})$' || true; } | while IFS= read -r target; do`,
